...
func NewProxy(db codeinteldb.DB, bundleManagerClient bundles.BundleManagerClient) (*httpapi.LSIFServerProxy, error) {
	return &httpapi.LSIFServerProxy{
		UploadHandler: http.HandlerFunc(uploadProxyHandler(db, bundleManagerClient)),
	}, nil
}

func uploadProxyHandler(db codeinteldb.DB, bundleManagerClient bundles.BundleManagerClient) func(http.ResponseWriter, *http.Request) {
	server := &Server{db: db, bundleManagerClient: bundleManagerClient}
